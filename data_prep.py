...
class MitralValveDataset(Dataset):
    ...
    def __init__(self, data_list, test_data=False,
    seq_len: int=16,
    ...
    dataset_type: DatasetType=DatasetType.EXPERT):
        ...
        if self.rotation_chance > 1e-12 and self.test_data:
            raise ValueError("Rotation chance is not supported for test data")
    ...
    def __getitem__(self, idx):
        ...
        result = {
            ...
            'label_idx' : sequence_label_idx,
            'video_name': item['name'],
            'orig_shape': item['video'].shape[:2],
        }

        # Optionally add original frames/mask/box if in test_data mode
        if self.test_data:
            ...
            result['orig_box']   = torch.from_numpy(np.expand_dims(box, axis=0))  # (1, H, W) -> (1, 1, H, W) for consistency
